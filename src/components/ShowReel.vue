<template>
  <gemini-scrollbar class="my-scroll-bar">
    ...
    <div class="find_admin">
      ...
      <el-button type="primary" icon="el-icon-search" size="small" @click="getShowreelFun()">搜索</el-button>
    </div>
      ...
      <template v-if="tableData.length>0">
        <el-table :data="tableData" stripe style="width: 100%" @selection-change="selectionChange" stripe
          ...
          <el-table-column prop="age" label="技术栈">
            <template slot-scope="scope">
              <div>
                <span v-for="item of scope.row.tecTag">{{item}}</span>
              </div>
            </template>
          </el-table-column>
          ...
          <el-table-column fixed="right" label="操作" width="150">
            <template slot-scope="scope">
              ...
              <el-button type="text" size="small" @click="delShowreel(scope.row._id)">移除</el-button>
            </template>
          </el-table-column>
        </el-table>
      </template>
      ...
      getAdminData(data) {
        ...
        if (resData.length > 0) {
          for (var i = 0; i < resData.length; i++) {
            ...
            resData[i].tecTag.length > 0 ? resData[i].tecTag = resData[i].tecTag : resData[i].tecTag = ['NULL'];
      ...
      linkToEdit(id) {
        this.$http({
          ...
          if (data.data.isAuth == 'auth') {
            this.$router.push({name: 'EditShowReel', params: {id: id}})
          } else {
            this.$notify({
              title: '警告',
              message: '权限不足，无法编辑随笔！',
